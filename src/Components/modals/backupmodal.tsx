...
const SubscriptionModal: React.FC<SubscriptionProps> = ({
  ...
  const supportedVersions: string[] = JSON.parse(
    import.meta.env.VITE_APP_SUPPORTED_VERSIONS || "[]",
  );

  // Validate the current version
  const isValidVersion = supportedVersions.includes(currentVersion);

  if (!isValidVersion) {
    console.error(
      `Invalid app version: ${currentVersion}. Supported versions are: ${supportedVersions.join(
        ", ",
      )}`,
    );
    return null;
  ...
  return (
    <AnimatePresence>
      ...
      >
        {currentVersion === supportedVersions[1] &&
        supportedVersions.includes(currentVersion) ? (
          <motion.div
            key="modal"
            ...
            className={`relative ${
              isPremium ? "h-auto w-[90%] max-w-md" : "h-[70vh] max-w-5xl"
          ...
          <motion.div
            ...
            transition={{ duration: 0.3, ease: "easeInOut" }}
            className={`relative ${
              isPremium ? "h-auto w-[90%] max-w-md" : "h-auto max-w-5xl"
            } overflow-y-auto rounded-lg bg-white p-6 shadow-lg will-change-[opacity,transform]`}
          >
            {/* Close button */}
            <button
            ...
            {/* Modal Description */}
            <p className="mb-6 text-center text-gray-600">
              "You’ve maximized the storage limit for your Free plan. Please
              manage your storage or contact support for additional options."
            </p>
